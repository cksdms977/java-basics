...
public class _05_Stream {
    public static void main(String[] args) {
        ...
        IntStream scoreSream = Arrays.stream(scores);

        String[] langs = {"python", "java", "javascript", "c", "c++", "c#"};
        Stream<String> langStream = Arrays.stream(langs);

        // Collection.stream() 하는 방법
        List<String> langList = new ArrayList<>();
//        langList.add("python");
//        langList.add("java");
        langList = Arrays.asList("python", "java", "javascript", "c", "c++", "c#");
        // System.out.println(langList.size());
        Stream<String> lagnListstream = langList.stream();
        ...
        Arrays.stream(scores).filter(x -> x >= 90).forEach(System.out::println); // 위의 문장과 똑같이 나옴
        ...
        // 4글자 이하 언어 중에서 c 라는 글자를 포함하는 언어가 하나라도 있는지 여부
        ...
        // 4글자 이하의 언어중에서 c라는 글자를 포함하는 언어를 대문자로 출력
